  ...
  (req, res) => {
    ...
    CarProfile.findOne({ user: req.params.user_id })
      ...
      .catch(err =>
        res
          .status(404)
          .json({ carprofile: "There is no car profile for this user" })
      );
  }
);
...
router.post(
  ...
  (req, res) => {
    ...
    CarProfile.findOne({ user: req.user.id }).then(carprofile => {
      if (carprofile) {
        //Update
        CarProfile.findOneAndUpdate(
          { user: req.user.id },
          { $set: CarProfileFields },
          { new: true }
        ...
        CarProfile.findOne({ handle: CarProfileFields.handle }).then(
          carprofile => {
            if (carprofile) {
            ...
            //Save CarProfile
            new CarProfile(CarProfileFields)
              .save()
              .then(carprofile => res.json(carprofile));
          }
        );
...
router.get(
  ...
  (req, res) => {
    CarProfile.findOne({ user: req.user.id })
      .then(carprofile => {
        //loop and add
        // console.log(carprofile.expense);

        var businesssum = 0;
        var personalsum = 0;

        carprofile.expense.forEach(expense => {
          if (expense.typeofexpense[0] === "Business") {
            businesssum += expense.totalamount;
          } else {
            expense.typeofexpense[0] === "Personal";
            personalsum += expense.totalamount;
          }
        });
        console.log({ totalbusinessexpenses: businesssum });

        console.log({ totalpersonalexpenses: personalsum });
        User.findOneAndUpdate(
          { _id: req.user.id },
          {
            totalbusinessexpenses: businesssum,
            totalpersonalexpenses: personalsum
          }
        ).then(
          res.json({
            totalbusinessexpenses: businesssum,
...
router.delete(
  ...
  passport.authenticate("jwt", { session: false }),
  (req, res) => {
    CarProfile.findOneAndRemove({ user: req.user.id }).then(() => {
